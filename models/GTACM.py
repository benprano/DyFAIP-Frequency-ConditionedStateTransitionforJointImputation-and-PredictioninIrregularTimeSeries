...
from spyder.api.widgets import auxiliary_widgets
from torch import Tensor

from diffusion_utils.embedding import AdaptiveNoiseEmbedding
...
class ATRUCell(torch.jit.ScriptModule):
    def __init__(self, input_size, hidden_size, seq_len):
        ...
        # 1. Feature-wise Decay Parameters
        self.freq_weight = nn.Parameter(torch.zeros(self.input_size))
        self.feat_slope = nn.Parameter(torch.zeros(self.input_size))
        self.global_gain = nn.Parameter(torch.ones(1))  # Replaces decay_attn for cleaner gain control
    ...
    def dyfaip(self, x: torch.Tensor, freq_dict: torch.Tensor, x_hidden: torch.Tensor):
        ...
        # 1. Sparsity Logic (Logit-space)
        mask = torch.isnan(x)
        sparsity_ratio = mask.float().mean()
        # Use softplus for strictly positive gain, preventing zero-gradient issues
        sparsity_gain = F.softplus(self.global_gain) * (1.0 - sparsity_ratio)
        # 2. Stable Slope Calculation
        # Clamp feat_slope to prevent extreme values that cause exponent explosion
        feat_slope = torch.clamp(F.softplus(self.feat_slope), min=0.01, max=5.0)
        exponent = -self.factor * (feat_slope * self.fixed_decay) * (1.0 + sparsity_gain)
        # 3. Numerically Stable Decay
        # Clamp exponent range to [-10, 0] to keep exp output between [0, 1] safely
        exponent = torch.clamp(exponent, min=-10.0, max=0.0)
        decay = torch.exp(exponent)

        # 5. Robust Normalization (Stable Max)
        # Adding epsilon prevents division by zero; using sum/mean normalization
        # is often more stable than hard max if input distributions vary.
        freq_raw = freqs * decay
        frequencies = freq_raw / (freq_raw.max() + 1e-7)
        ...
        return decay, frequencies, gate, imputed_missed_x, x_imputed
    ...
    def forward(self, inputs, times, last_values, freqs):
        ...
        for layer_idx, (f_cell, b_cell) in enumerate(self.layers):
            ...
            if self.bidirectional:
                ...
                lambda_weights = torch.cat((lambda_weights, lambda_weights_b), dim=0)
        ...
        #concatenate_inputs = torch.cat((hidden_his, imputed_inputs.permute(0, 1, 3, 2)), dim=-1)
