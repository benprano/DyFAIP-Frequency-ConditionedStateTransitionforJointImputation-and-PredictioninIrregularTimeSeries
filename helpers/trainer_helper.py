...
from helpers.metrics import TrainerMetrics, EarlyStopping, EarlyStoppingDualObjective
...
class ModelTrainer:
    ...
    def _forward_pass(self, model: torch.nn.Module,
        ...
        #print("indices", indices[0], indices[0].shape, torch.stack(indices).shape)
    ...
    def evaluate_model(self, model_class: type, model_path: str,
        ...
        real_data, data_with_missingness, indicies_inputs = [], [], []
        with torch.no_grad():
            progress_bar = tqdm(
                test_loader,
                desc="Evaluating",
            ...
            for batch_idx, batch_data in enumerate(progress_bar):
                ...
                (outputs, weights_freq, h_bar, c_bar, confidence,
                 lambda_freq, frequencies, gate_imputs,uncertainty,
                ...
                real_data.append(temporal_features.cpu().detach().numpy())
                data_with_missingness.append(data_with_missing.cpu().detach().numpy())
                indicies_inputs.append(torch.stack(indices).cpu().detach().numpy())
        ...
        return (np.hstack(real_inputs), np.hstack(imputed_inputs), np.vstack(freq_weights),
                ...
                np.vstack(uncertainty_weights),np.vstack(imp_imputs_batch), np.vstack(real_data),
                np.vstack(data_with_missingness), np.hstack(indicies_inputs), np.vstack(all_targets),
                np.vstack(all_outputs))
    def inference(self, model_class: type, model_path: str,
        ...
        (fin_inputs_i, inputs_outputs_i, freq_weights, h_bar_weights,
         ...
         gate_imputs_weights,uncertainty_weights,imp_imputs_batch,real_data,
         data_with_missingness, indicies_inputs , y_pred, y_true)= self.evaluate_model(
        ...
        return ([y_true], [y_pred], [fin_inputs_i],
                ...
                [gate_imputs_weights], [imp_imputs_batch],
                [real_data],[data_with_missingness],
                [indicies_inputs],[uncertainty_weights])
    ...
    def train_validate_evaluate(self, model_class: type, model: torch.nn.Module,
        ...
        early_stopping = EarlyStopping(
            ...
            primary_threshold=0.0001,
            imp_threshold=0.00001,
            combined_threshold=0.0001,
            primary_weight=0.5,
            imp_weight=0.5,
            logger=logger
        )
        ...
        return self._final_evaluation(
            model_class, early_stopping.combined_save_path, test_loader, val_targets, val_outputs,
    ...
    def _final_evaluation(self, model_class: type, model_path: str,
        ...
        if self.is_classification:
            ...
            logger.info(f"Validation MSE: {val_mse}, MAE: {val_mae}")
       ...
       # ================================================================
        ...
        (targets_list, outputs_list,
         ...
         imp_imputs_batch,real_data,data_with_missingness,
         indicies_inputs, uncertainty) = self.inference(model_class, model_path, test_loader)
        ...
        if self.is_classification:
            # print(f"[INFO] len {len(test_targets)} {test_targets[0]}, {test_outputs[0]}")
            binary_scores = self.metrics.compute_binary_metrics([test_targets[0]],
                                                                [test_outputs[0]])
            ...
            np.savez(
                ...
                real_timeseries_data=real_data, real_timeseries_data_missingness=data_with_missingness,
            ...
            print(f"[INFO] len {len(test_targets)}")
            ...
            np.savez(
                ...
                real_timeseries_data=real_data,real_timeseries_data_missingness=data_with_missingness,
                missingness_indicies=indicies_inputs,lambda_weight=lambda_weights,frequencies_weight=frequencies_weights,
                gate_imputs=gate_imputs_weights, imputs_batch_imputation=imp_imputs_batch,
                confidence_weigths=confidence, uncertainty_weights=uncertainty
            )
            np.savez(
            ...
            self._save_scores_to_json(scores, save_path, f'scores_{model_name}')
        ...
        del (test_targets, test_outputs, real_inputs, imputed_inputs,
             ...
             gate_imputs_weights,imp_imputs_batch, confidence, uncertainty, real_data,
             data_with_missingness,indicies_inputs)
        gc.collect()
        if torch.cuda.is_available():
            torch.cuda.empty_cache()
